...
Card2 supports multiple images with an interactive hover gallery. When you pass an `images` array with more than one image, the card displays invisible vertical hover zones. Moving your mouse across the image area switches between images, similar to Instagram or Tinder photo galleries.
...
<div className="overflow-x-auto">
  <div className="min-w-xl">
    ...
    | `image` | `string` \| [Image](/docs/image) | - | Path to the card's image (single image) |
    | `images` | [Image](/docs/image)[] | - | Array of images for multi-image hover gallery |
...
- When both `image` and `images` props are provided, `images` takes precedence if it has more than one item
- If `images` array has only one item, it behaves like the single `image` prop
