...
<!-- Array props (position defaults to the array index) -->
<Breadcrumbs
  items={[
...
<div className="overflow-x-auto">
  <div className="min-w-xl">
    ...
    | `items` | `BreadcrumbsItemData[]` | `[]` | Renders the trail from an array instead of `<BreadcrumbsItem>` children. Accepts every `BreadcrumbsItem` prop plus `icon` (icon name); `position` defaults to the array index + 1. Children are ignored when `items` is non-empty. |
  </div>
</div>
...
<div className="overflow-x-auto">
  <div className="min-w-xl">
    ...
    | `position` | `number` | **Required** | The position of this item in the breadcrumb trail (required for valid schema.org BreadcrumbList markup) |
