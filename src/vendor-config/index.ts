  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('vendor');

        const virtualModuleId = 'vendor:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-vendor-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`Vendor \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info('Vendor config has been loaded.');
        }
      },
